...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable('Line_MLB', (table) => {
    table.increments('id');
  ...
  await knex.schema.createTable('Line_NFL', (table) => {
    table.increments('id');
    table.string('home_team').notNullable();
    table.string('away_team').notNullable();
    table.string('home_line').notNullable();
...
export async function down(knex: Knex): Promise<void> {
  await knex.schema.dropTable('Line_MLB');
  await knex.schema.dropTable('Line_NFL');
}
